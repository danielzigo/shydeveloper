...
import { resetScore, setSelected, updateScore } from "@/features/gameSlice";

const GameResult = () => {
	const dispatch = useDispatch();
	const { selected, advancedMode, score } = useSelector((state: RootState) => state.game);
	...
	return (
		<div className="results-wrapper relative">
			{/* Countdown Display */}
			{countdownIndex >= 0 && (
				<div className={`${styles.countdownWrapper} flex h-full items-center justify-center`}>
			...
			<div className={showResult ? styles.results : styles.pending}>
				...
				{countdownIndex === -1 && showResult && (
					<div className={styles.resultsSelectionWrapper}>
						<div className={styles.selection}>
							<h3>You Chose</h3>
							<motion.div className={result === "Win" ? styles.win : styles.computer}>
								<GameButton
									key={`player-${selected}`}
									classes={`${styles2[PlayRules[selected]?.value] || ""} ${styles.btn}`}
						...
						<div className={styles.selection}>
							...
							<motion.div className={result === "Lose" ? styles.win : styles.computer}>
								{computer !== null && (
									<GameButton
										key={`computer-${computer}`}
										classes={`${styles2[PlayRules[computer]?.value] || ""} ${styles.btn}`}
				...
				{showResult && (
					<div className={styles.result}>
						<motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 1 }}>
